...
def detect_watermark(frames: List[np.ndarray], keywords: str = None) -> Optional[List[Dict[str, int]]]:
    ...
    try:
        ...
        prompt = f"""
SYSTEM ROLE:
You are the Hybrid Watermark Geometry Authority.
Your goal is PIXEL-PERFECT detection of unwanted overlays.

TASK:
Analyze these {len(frames)} frames. Return a JSON list of detected watermarks.
For each watermark, provide bounding box, visual properties, and MOTION BEHAVIOR.

STRICT RULES:
1. BOX 2D: Must be [ymin, xmin, ymax, xmax] (0-1000 scale).
2. ANCHORING: Where is it visually anchored? (top_left, bottom_right, floating, etc).
3. TYPE: "logo" (symbol), "text" (handle/@name), or "mixed".
4. MOTION: "static" (fixed), "rigid_motion" (bouncing/sliding), or "dynamic" (fading/changing shape).
...
"""
        ...
        # MULTI-FRAME ANALYSIS (Deep Scanning)
        # Send ALL sampled frames (Start, Mid, End) to catch "Slow Reveal" watermarks.
        request_contents = b64_contents
        
        # 2. Simplified Prompt (Multi-View)
        prompt = f"""
        Find the watermark (logo or text) in these images.
        If it appears in ANY image, report it.
        TARGETS: {keywords if keywords else "Any visible logo/text overlay"}
...
def verify_watermark(frame: np.ndarray, candidate_box: Dict[str, int]) -> Optional[bool]:
    ...
    try:
        ...
        prompt = """
        Analyze this crop. Confirm if there is a digital overlay, logo, or text handle (e.g. @name) in the CENTER.
