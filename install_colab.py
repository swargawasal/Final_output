...
NUMPY_CANDIDATES = [
    "2.0.0",   # Newest (Py3.9+)
    "1.26.4",  # Stable Legacy (Py3.11/3.12 safe)
    "1.25.2",
    "1.24.4",
    "1.23.5"
]
...
def ensure_venv():
    ...
    v = sys.version_info
    log(f"System Python Detected: {v.major}.{v.minor}.{v.micro}", "INFO")

    if not os.path.exists(VENV_DIR):
        log(f"Creating virtual environment in '{VENV_DIR}' (System Packages: Enabled)...", "ACTION")
        # CRITICAL FIX: Use --system-site-packages to inherit Colab's pre-installed CUDA/Torch/JAX
        # This prevents "pip install torch" which downloads massive files and often breaks GPU linkage.
...
def install_deps():
    ...
    if os.path.exists(req_file):
        ...
        # We perform a robust install.
        # If numpy/opencv mismatch happens later, 'auto_heal' will fix it.
        run_cmd([PIP_EXEC, "install", "-r", req_file], "Dependency Installation", check=False)
    else:
...
def check_ABI_health():
    ...
    """
    check_script = "import cv2; import numpy; print(f'Imports Successful: CV2 {cv2.__version__} | NP {numpy.__version__}')"
    try:
        subprocess.run(
...
def reinstall_opencv():
    ...
    # Using --no-cache-dir to prevent picking up bad cache
    run_cmd([PIP_EXEC, "install", "--no-cache-dir", "--force-reinstall", "opencv-python-headless"], "Reinstalling OpenCV-Headless")
...
def auto_heal_numpy(retry_opencv_fallback=True):
    ...
    # Filter candidates based on Python version
    v_maj = sys.version_info.major
    v_min = sys.version_info.minor
    
    candidates = NUMPY_CANDIDATES
    
    # Py3.11/3.12 cannot use older numpys easily (often compile fails)
    # Stick to newer ones first.
    if v_min >= 11:
         candidates = [c for c in NUMPY_CANDIDATES if not c.startswith("1.23") and not c.startswith("1.24")]
         log(f"Python {v_maj}.{v_min} detected: Pruned old NumPy candidates.", "INFO")
    ...
    for version in candidates:
        ...
        try:
             # Try install
            run_cmd([PIP_EXEC, "install", f"numpy=={version}"], f"Installing NumPy {version}", check=True)
        except Exception:
             log(f"Failed to install numpy=={version} (Compile Error?)", "WARN")
             continue
    ...
    # Don't exit, maybe it runs anyway?
    # sys.exit(1)
...
def patch_basicsr():
    ...
    try:
        ...
        # Fallback for venv
        site_pkg = os.path.join(VENV_DIR, "lib", f"python{sys.version_info.major}.{sys.version_info.minor}", "site-packages")
        if not os.path.exists(site_pkg): return
