...
class TemporalSmartRefiner:
    ...
    def refine_box_temporal(video_path, rough_box):
        ...
        try:
            ...
            consensus_map = np.zeros((h, w), dtype=np.uint8)
    ...
    if StaticPatchReuseEngine and mask_paths:
        ...
        if determined_mode in ["static", "rigid_motion"]:
            ...
            if os.getenv("COMPUTE_MODE", "auto") == "cpu":
                ...
                # NEW: CPU_SAFE_DYNAMIC_FEATHER MODE
                # If we are here, it means:
                # 1. We are in CPU Mode
                # 2. Strategy is DYNAMIC (so Static Patch was rejected or not applicable)
                
                # Rule 2: Mask Merge & Suppression
                safe_masks = _resolve_cpu_safe_mask_priority(mask_paths)
                
                logger.info("🧯 Dynamic Feather Mode: All expansions & polish disabled")
        ...
        while True:
            ...
            if cv2.countNonZero(final_mask) > 0:
                ...
                if rw > 0 and rh > 0:
                    ...
                    # Check if this specific ROI overlaps a face in this specific frame
                    # This handles camera motion where a static mask might drift onto a face.
                    
                    roi_box = {'x': rx, 'y': ry, 'w': rw, 'h': rh}
                    is_safe, reason = FaceProtector.is_safe_region(frame, roi_box)
                    
                    if not is_safe:
                        # Skip inpainting for this frame (copy original)
                        # effectively zeroing the mask influence
                        # Log once per second to avoid spam
                        if frame_idx % 30 == 0:
                             logger.warning(f"🛡️ Dynamic Face Guard: Creating Safe Zone at frame {frame_idx} ({reason})")
                        
                        # Write original frame (no inpaint)
                        out.write(frame)
                        frame_idx += 1
                        continue
