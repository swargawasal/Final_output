...
def _run_command(cmd: List[str], check: bool = False, timeout: int = None) -> bool:
    try:
        ...
        err_msg = e.stderr[:300] if e.stderr else "No stderr"
...
def normalize_video(input_path: str, output_path: str, target_res: tuple = (1080, 1920), max_duration: float = None, resize: bool = True):
    ...
    if encoder == "libx264": cmd.extend(["-crf", "23"])
...
def apply_ferrari_composer(
    ...
    # Filter Strings
    f_scale = f"scale={w}:{h}:force_original_aspect_ratio=decrease,pad={w}:{h}:(ow-iw)/2:(oh-ih)/2"
    f_ramp = (
        f"setpts='if(lt(T,2),PTS/{slow},"
        f"if(lt(T,6),PTS,"
        f"PTS/{fast}))'"
    )
    
    # --- COLOR GRADING LOGIC ---
    # 🎬 CINEMATIC / DAVINCI-LIKE (SAFE)
    f_cinematic = (
    ...
    # 📸 PAPARAZZI / CLEAN CELEB LOOK
    f_paparazzi = (
        f"curves=all='0/0.04 0.25/0.22 0.5/0.55 0.8/0.92 1/1',"
        f"eq=contrast={1.08 + color_intensity*0.1}:"
        f"saturation={1.01 + color_intensity*0.05}:brightness=0,"
        f"colorbalance=rh={0.03*color_intensity}:gh={0.02*color_intensity}:bh={-0.01*color_intensity},"
        f"unsharp=5:5:{sharpen}"
    )

    # 🌑 DARK CINEMA / NETFLIX-STYLE
    f_dark_cinema = (
        f"curves=all='0/0.03 0.4/0.35 0.7/0.75 1/0.95',"
        f"eq=contrast={1.1 + color_intensity*0.15}:"
        f"saturation={0.85 + color_intensity*0.05},"
        f"colorbalance=rs={-0.03*color_intensity}:"
        f"gs={-0.03*color_intensity}:"
        f"bs={-0.02*color_intensity}"
    )
    ...
    # Chain: [0:v] -> Scale -> Ramp -> Color -> [v_out]
    video_filter_chain = f"{f_scale},{f_ramp},{f_color}"
    ...
    # 2. LOAD FALLBACK MUSIC (If Requested)
    # Only if enabled in config
    if FALLBACK_AUDIO:
        fallback_track = _get_next_music_track()
        if fallback_track:
            inputs.extend(["-stream_loop", "-1", "-i", fallback_track]) # Loop music (Requires -shortest)
            idx_music = next_idx
            next_idx += 1
            logger.info(f"🎵 Fallback Music Selected: {os.path.basename(fallback_track)}")
    ...
    # Strategy: Gated Sidechain + Exclusive Source.
    # 1. Select ONE Source: Original OR Music (Strict user request: "Do not mix").
    # 2. Hard Duck: Crush Source during VO so it sounds like it "Spawns Up" after.
    
    # 3a. Exclusive Source Selection
    bg_source_label = None
    
    # Logic: Priority to KEEP_ORIGINAL
    # Logic: Priority to KEEP_ORIGINAL
    if KEEP_ORIGINAL_AUDIO and has_source_audio:
         # Standardize volume AND Format (CRITICAL: Fixes silent/crash issues)
    ...
    elif FALLBACK_AUDIO and idx_music != -1:
         # Use Fallback Music ONLY if Original is disabled/missing
         m_vol = MUSIC_VOLUME
    ...
    if idx_vo != -1:
        ...
        f_graph.append(f"[vo_ctrl_raw]agate=threshold=0.05:ratio=9000:range=-90dB:attack=1:release=50[vo_ctrl_gated]")
        
        # Apply HARD Sidechain (The "Spawn Up" Effect)
        # Ratio 20 = Limiter/Hard Duck (Crushes BG to silence during VO)
        # Release 300ms = Fast Swell (Spawn)
        f_graph.append(f"{bg_source_label}[vo_ctrl_gated]sidechaincompress=threshold=0.01:ratio=20:attack=5:release=300[bg_ducked]")
    ...
    if count == 1:
        ...
        # Amerge (Summing) guarantees no volume loss
        merge_str = "".join(final_inputs)
        f_graph.append(f"{merge_str}amerge=inputs={count}[final_merged]")
        
        # Pan to Stereo (Sum)
        pan_clause = "c0<c0"
        if count >= 2: pan_clause += "+c2"
        pan_clause += "|c1<c1"
        if count >= 2: pan_clause += "+c3"
            
        f_graph.append(f"[final_merged]pan=stereo|{pan_clause}[a_out]")
    ...
    cmd.extend([
        ...
        "-shortest", # PREVENTS HANG: Stops encoding when the shortest stream (Video) ends.

        output_path
    ])
    
    if encoder == "libx264": cmd.extend(["-crf", REENCODE_CRF])
    else: cmd.extend(["-rc", "vbr", "-cq", "19"])
...
def compile_with_transitions(input_video: Path, title: str, aggressive_watermark: bool = False, force_reprocess: bool = False, retry_level: int = 0) -> Tuple[Optional[Path], Dict]:
    ...
    try:
        ...
        # 1. Watermark Check
        if not wm_context or not wm_context.get('removal_success'):
            t_score -= 50
        ...
        if force_reprocess and t_score < 30: # Lowered threshold from 70
            abort_reason = f"Low Transformation Score ({t_score}) on Collision"
            
        if abort_reason:
            logger.error(f"❌ MONETIZATION ABORT: {abort_reason}")
            run_metrics['monetization'] = {'risk_level': 'CRITICAL', 'reason': abort_reason}
            _save_sidecar(final_dest, caption_meta={}, pipeline_metrics=run_metrics)
        ...
        _save_sidecar(final_dest, caption_meta={}, pipeline_metrics=run_metrics) # Update final status logic if needed
...
def compile_batch_with_transitions(video_files: List[str], output_filename: str) -> Optional[str]:
    ...
    try:
        ...
        if total_raw_duration > 65 or total_raw_duration <= 0.1:
             ...
             if total_raw_duration <= 0.1:
                 ...
                 logger.info("🎬 Mode: LONG FORM (Total > 65s). Disabling trims & Rebuilding Audio.")
                 logger.info("ℹ️ NOTE: YouTube now classifies vertical videos up to 3 MINUTES as Shorts. This upload may still be a Short.")
             max_per_clip = None 
             
             # --- AUDIO REBUILD (LONG FORM ONLY) ---
             # Logic is handled inside the loop now
             pass
             
        else:
             # SHORTS MODE (Aggressive Trim to 30s as requested)
             logger.info("⚡ Mode: SHORTS (Total <= 65s). Trimming to 30s for fast pacing.")
        ...
        for i, v_path in enumerate(video_files):
            ...
            if normalize_video(v_path, norm_path, target_res, max_duration=max_per_clip):
                ...
                if total_raw_duration > 65:
                     ...
                     # STRICT AUDIO REMODE: ALWAYS REMIX if Long Form
                     should_remix = True
                else:
                     # Shorts
                     if ENABLE_HEAVY_REMIX_SHORTS: should_remix = True
                ...
                if should_remix:
                     try:
                         ...
                         if not os.path.exists(meta_path):
                             base, _ = os.path.splitext(v_path)
                             meta_path = base + ".json"
                         
                         if not os.path.exists(meta_path):
                             # Fallback: maybe in downloads dir if v_path is temp?
                             pass

                         rebuilt_path = os.path.join(job_dir, f"rebuilt_{i:03d}.mp4")
                         
                         # Playlist Selection (Unified)
                         # Initialize playlist if not done
                         if 'music_playlist' not in locals():
                             music_playlist = glob.glob(os.path.join("music", "*.mp3"))
                             if music_playlist: 
                                 random.shuffle(music_playlist)
                                 logger.info(f"🎵 Music Playlist Initialized: {len(music_playlist)} tracks")
                             music_idx_ptr = [0] # Mutable pointer
                             
                         # CALL REBUILDER
                         # Note: passing music_idx_ptr which is a list [int] to allow modification
                         is_last_clip = (i == len(video_files) - 1)
                         # Smart Fade: If not last clip, fade is minimal (0.05) to allow crossfade to catch it.
                         # If last clip, standard fade out (2.0)
                         fade_dur = 2.0 if is_last_clip else 0.05
                         
                         if _rebuild_clip_audio(norm_path, rebuilt_path, meta_path, music_playlist, music_idx_ptr, fade_out_duration=fade_dur):
                             norm_files.append(rebuilt_path)
                             continue 
                         else:
                             # If rebuild fails (e.g. no caption), we MUST STILL STRIP AUDIO for Long Form
                             if total_raw_duration > 65:
                                  # Force silence strip
                                  logger.warning("⚠️ Audio Remix Failed. Stripping audio strictly.")
                                  # Re-encode with silence
                                  silent_path = os.path.join(job_dir, f"silent_{i:03d}.mp4")
                                  cmd_silence = [
                                      FFMPEG_BIN, "-y", "-i", norm_path,
                                      "-f", "lavfi", "-i", "anullsrc=channel_layout=stereo:sample_rate=44100",
                                      "-map", "0:v", "-map", "1:a",
                                      "-c:v", "copy", "-c:a", "aac", "-shortest",
                                      silent_path
                                  ]
                                  if _run_command(cmd_silence):
                                      norm_files.append(silent_path)
                                  else:
                                      norm_files.append(norm_path) # Critical fail
                             else:
                                  norm_files.append(norm_path)
                     except Exception as e:
                         logger.warning(f"⚠️ Audio Remix Skipped ({e}).")
                         if total_raw_duration > 65:
                              # Try to at least strip audio?
                              pass
                         norm_files.append(norm_path)
                else:
                    norm_files.append(norm_path)
        ...
        try:
            ...
            output_filename = _inject_branding(output_filename)
