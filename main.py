...
async def safe_reply(update: Update, text: str):
    ...
    if not user_limiter.check(user_id):
        ...
        return # Silent drop or maybe log
...
async def initiate_compilation_title_flow(update: Update, merged_path: str, n_videos: int, hashtags: str):
    """
    New Flow: 
    ...
    if gemini_mode == "on":
        try:
             ...
             await safe_reply(update, "🧠 Generating Compilation Title via Gemini...")
             generated_title = await asyncio.to_thread(
                 gemini_captions.generate_compilation_title, 
                 n_videos
             )
        except Exception as e:
    ...
    if generated_title:
        ...
        await finish_compilation_upload(update, merged_path, generated_title, hashtags, n_videos=n_videos)
        return
    ...
    try:
        if os.path.exists("title_expansion_presets.json"):
            ...
            if presets:
                msg_lines = ["📌 Select title expansion :"]
                # Ensure sorted keys
    ...
    if presets_msg:
        ...
        fallback_title = f"Compilation {n_videos} Videos"
        await finish_compilation_upload(update, merged_path, fallback_title, hashtags, n_videos=n_videos)


async def finish_compilation_upload(update: Update, merged_path: str, title: str, hashtags: str, n_videos: int = 10):
    """
    ...
    # User Request: NEVER send compilation to Telegram (file too big/spammy).
    # Upload to YouTube ONLY if configured.
    
    if not send_to_youtube:
        # Just notify about local save
        await safe_reply(update, f"✅ Compilation saved locally:\n`{merged_path}`")
        return
    ...
    try:
        ...
        if link and os.getenv("ENABLE_COMMUNITY_POST_COMPILATION", "yes").lower() == "yes":
            # Schedule in background (don't await/block)
            logger.info("🚀 Triggering Community Promotion (Background Task)...")
            asyncio.create_task(
                community_promoter.promoter.schedule_promotion(
                    uploader._get_service_sync(), # Re-using sync service logic carefully
                    link, 
                    n_videos,
                    delay_seconds=random.randint(120, 300) # 2-5 min delay
                )
            )
            
    except Exception as e:
        logger.exception("Upload failed: %s", e)
        await safe_reply(update, f"❌ Upload failed: {e}")
...
async def compile_last(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # If user provided a name query, maybe use that as title directly? 
        # Requirement says: "I want you to use gemini's 1 request... or fallback to ask user... for compilation"
        # Since /compile_last is a compilation, we should PROBABLY apply the same logic.
        # BUT if user explicitly gave a name (name_query), maybe we respect that?
        # User said: "only for compliation, even if its gemini's title genration or this title asking."
        # This implies standard logic applies.
        # However, if I typed `/compile_last 6 reem`, I probably want "reem" in the title.
        # I'll prioritize name_query if present, else use the new flow.
        
        if name_query:
            caption = f"🎬 Compilation: {name_query}"
            await finish_compilation_upload(update, merged, caption, comp_hashtags)
        else:
            # New Flow
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'WAITING_FOR_TITLE':
        ...
        # Generate Unique ID from URL to prevent filename collision/cache bleeding
        # (Crucial for "Force Fresh Instance" logic)
        import hashlib
        ...
        # Usage: force_filename overrides title logic to ensure unique path on disk
        ...
        if col_type != "NONE":
            ...
            # We already have a unique filename (from url_hash), so caches are naturally cold.
            # Just Ensure we don't accidentally load a 'similar' json.
            # Explicitly touch/clean metadata?
            meta_path = str(video_path) + ".json"
            if os.path.exists(meta_path):
                 # If metadata exists from a PREVIOUS run of this specific URL/Hash combo, 
                 # we ideally want to nuke it if we are "Forcing Fresh"?
                 # But if ID is same (re-download), maybe reusing is ok?
                 # User: "Force Reprocess... skip ALL cached outputs".
                 # So if re-downloading same URL, we should probably Wipe cache too.
                 try: os.remove(meta_path)
                 except: pass
        
        # Register right away (or after? "Only trust in order").
        # Registering now prevents parallel exploits?
        # DedupEngine.register_content(url_hash, video_path) 
        # Better to register at SUCCESS to avoid polluting index with failed jobs?
        # But we want to block the content. 
        # Let's register NOW.
    ...
    if state == 'WAITING_FOR_COMPILATION_TITLE':
        ...
        try:
             ...
             if title_choice in presets:
                 label = presets[title_choice]['label']
                 # We construct the title: "Label - Compilation"
                 # Or just "Label" ? User sample: "1️⃣ Airport Look" -> proceed with that value as title.
                 # So title = "Airport Look"
                 final_title = label
             else:
                 await safe_reply(update, "⚠️ Invalid selection. Please reply with the number (e.g., '1').")
                 return
        except Exception as e:
             logger.error(f"Preset load error: {e}")
             final_title = f"Compilation {n_videos} Videos" # Fallback
...
async def verify_watermark(update: Update, context: ContextTypes.DEFAULT_TYPE, is_positive: bool = None):
    ...
    with get_session_lock(user_id):
        session = user_sessions.get(user_id, {})
        if not session or 'pending_video' not in session:
            msg = "❌ Session expired. Please upload again."
            if query: await query.edit_message_text(msg)
            else: await safe_reply(update, msg)
            return

        video_path = session['pending_video']
        ...
        if is_positive:
            ...
            msg = f"✅ Awesome! Processing '{title}'..."
            if query: await query.edit_message_text(msg)
            else: await safe_reply(update, msg)
            ...
            if retry_count > 2:
                ...
                await query.edit_message_text("❌ Maximum retries reached. I'm sorry I couldn't clean it.")
                user_sessions.pop(user_id, None)
                GlobalState.set_busy(False)
                return
            ...
            await query.edit_message_text(f"🔄 Retry {retry_count}/2: Activating {mode_name} Correction...\n(This might take longer)")
...
def main():
    ...
    # HARDENED TIMEOUTS
    app = ApplicationBuilder().token(TELEGRAM_BOT_TOKEN).connect_timeout(60).read_timeout(60).write_timeout(60).pool_timeout(60).build()
                    ...
                    try:
                        if os.path.getmtime(item_path) < cutoff:
                            if CLEANUP_POLICY == "immediate":
                                ...
                                # For strict/delayed, we might just log or move
                                os.remove(item_path)
                                count += 1
                    except Exception as e:
